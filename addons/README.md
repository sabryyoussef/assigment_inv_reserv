...
Insights
    ...
    assigment_inv_reserv/assigment

/stock_reservation_engine/
tT
sabryyoussef
sabryyoussef
Standardize addon layout for Odoo SH
32d6136
 · 
2 hours ago
This branch is 6 commits behind main.
...
Finalize assignment delivery and documentation
	
2 hours ago
data
	
fix: remove unused currency_id field, fix batch state logic, delete e…
...
feat(stock_reservation_engine): demo inventory, screenshots under sta…
...
stock_reservation_engine: tests, HTTP API fixes, security, TEST_REPORT
...
stock_reservation_engine: picking-safe moves action, docs, QA script
	
8 hours ago
views
	
fix: align allocate button access with reservation user permissions
	
4 hours ago
Odoo_Stock_Reservation_Engine.pdf
	
docs: add Odoo Stock Reservation Engine PDF
...
feat(stock_reservation_engine): demo data, docs, modern chatter, stoc…
...
Assets live under static/description/screenshots/. Capture tooling: tools/screenshots/ (npm run capture, npm run walkthrough). Index: static/description/screenshots/README.md.
Scope delivered
...
Functional flow
    ...
    Add lines (product, requested qty, location; optional preferred lot).
    Confirm the batch.
    Allocate (UI or API): engine walks stock.quant, updates allocated_qty and line state.
    For lines with allocated_qty > 0, create or refresh stock.move (destination = staging rule above).
    Generate pickings: group moves without picking_id; create internal Transfers, confirm, link to picking_ids.
    In Inventory, open the Transfer when needed; optional Check Availability, then Validate to post stock moves.
    Open Stock Reservations → Dashboard to review line-level outcomes (states, requested vs allocated, product breakdown in pivot).
...
How to verify
    ...
    Upgrade module Stock Reservation Engine (Apps → Upgrade, or -u stock_reservation_engine).
    Restart the Odoo process if Python files were changed (reloads bytecode).
    Open Inventory → Stock Reservations → Reservation Batches (or a demo batch).
    Confirm, then Allocate.
    On the batch form, open Transfers — expect at least one internal transfer when stock was allocated; origin matches batch name.
    Cancel the batch and confirm linked non-done transfers move to the cancelled state as well.
    Allocate again on a partial batch (requested qty > on hand): picking count and picking ids must stay stable (no duplicate picking for already-linked moves).
    Dashboard: Stock Reservations → Dashboard — confirm graph shows bars by state (and stacked product); switch to pivot; check requested vs allocated for products and states (filters in search apply to both views).
...
Critical queries
    ...
    stock.quant search on product_id, location_id child_of, company_id, and optional lot_id
    Internal picking type fallback lookup on stock.picking.type
    Hashed token lookup on reservation.api.token
...
Installation
    ...
    Addons path contains stock_reservation_engine.
    Update apps list; install Stock Reservation Engine.
    Assign Stock Reservation User or Stock Reservation Manager.
    Create API tokens if needed.
...
assigment_inv_reserv/assigment/stock_reservation_engine at clean-odoo-sh · sabryyoussef/assigment_inv_reserv